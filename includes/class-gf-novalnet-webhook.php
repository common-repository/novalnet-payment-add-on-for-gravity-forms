...
class GF_Novalnet_Webhook {
	...
	public function handle_transaction_cancel() {
		...
		$this->response['message'] = GF_Novalnet_Helper::format_text( sprintf( __( 'The transaction has been canceled on %1$s %2$s.', 'novalnet-gravity-forms' ), GF_Novalnet_Helper::formatted_date(), current_time( 'H:i:s' ) ) );
	...
	public function handle_transaction_update() {
		...
		if ( in_array( $this->event_data['transaction']['status'], array( 'PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED' ), true ) ) {
			...
			if ( 'DEACTIVATED' === $this->event_data['transaction']['status'] ) {
				/* translators: %s: Date */
				$this->response['message'] = sprintf( __( 'The transaction has been canceled on %1$s %2$s.', 'novalnet-gravity-forms' ), GF_Novalnet_Helper::formatted_date(), current_time( 'H:i:s' ) );
				$transaction_comments      = GF_Novalnet_Helper::form_transaction_details( $this->event_data );
	...
	public function handle_subscription_suspend() {
		...
		$this->response['message'] = sprintf( __( 'This subscription transaction has been suspended on %s', 'novalnet-gravity-forms' ), current_time( 'Y-m-d H:i:s' ) );
	...
	public function handle_subscription_reactivate() {
		...
		$this->response['message'] = sprintf( __( 'Subscription has been reactivated for the TID:%1$s on %2$s %3$s. Next charging date :%4$s', 'novalnet-gravity-forms' ), $this->parent_tid, GF_Novalnet_Helper::formatted_date(), current_time( 'H:i:s' ), $this->event_data ['subscription']['next_cycle_date'] );
